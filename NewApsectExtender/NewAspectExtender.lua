...
function AspectExtender.GetRefundableBlood( weaponName, itemIndex )
    ...
    local upgrade = WeaponUpgradeData and WeaponUpgradeData[weaponName] and WeaponUpgradeData[weaponName][itemIndex]
    local costs = upgrade and upgrade.Costs
    if not costs or level <= 0 then
        return 0
    end
    ...
    for i = 1, level do
        ...
        if type(entry) == "table" then
            bloodCost = bloodCost + (entry.SuperLockKeys or 0)
        elseif type(entry) == "number" then
            bloodCost = bloodCost + entry
        end
    end
    return bloodCost
end
...
function AspectExtender.GetRefundKeyAmount( weaponName, itemIndex )
	local cfg = AspectExtender.BloodRefundConfig
	local perBlood = cfg and tonumber(cfg.KeyCostPerBloodRefunded) or 0
	if perBlood < 0 then
		perBlood = 0
	end
	if cfg and cfg.FreeMode then
		return 0
	end
	if not WeaponUpgradeData or not WeaponUpgradeData[weaponName] or not WeaponUpgradeData[weaponName][itemIndex] then
		return 0
	end	
	local level = GetWeaponUpgradeLevel( weaponName, itemIndex ) or 0
	if level <= 0 then
	...
	local bloodSpent = 0
	if AspectExtender.GetRefundableBlood then
		bloodSpent = AspectExtender.GetRefundableBlood( weaponName, itemIndex )
	end
	if bloodSpent < 0 then
		bloodSpent = 0
	end
	local total = perBlood * bloodSpent
...
ModUtil.Path.Override( "ShowWeaponUpgradeScreen", function (args)
	...
	for index, _ in pairs( WeaponUpgradeData[weaponName] ) do
		bloodCost = bloodCost + AspectExtender.GetRefundableBlood( weaponName, index )
		keyCost = keyCost + AspectExtender.GetRefundKeyAmount( weaponName, index )
	end
	if bloodRefundConfig and bloodRefundConfig.FreeMode then
        keyCost = 0
	end
	print( "Refund debug: blood = " .. tostring(bloodCost) .. " keys = " .. tostring(keyCost))

	components.RefundButton = CreateScreenComponent({ Name = "ButtonRefund", Scale = 1, Group = "Combat_Menu_TraitTray" })
    Attach({ Id = components.RefundButton.Id, DestinationId = components.ShopBackground.Id, OffsetX = 630, OffsetY = -350 })
    components.RefundButton.OnPressedFunctionName = "RefundBlood"
    components.RefundButton.BloodRefundCost = bloodCost
    components.RefundButton.KeyCost = keyCost
    components.RefundButton.Weapon = weaponName

	components.RefundTotalLabel = CreateScreenComponent({ Name = "BlankObstacle", Group = "Combat_Menu_TraitTray" })
	Attach({ Id = components.RefundTotalLabel.Id, DestinationId = components.ShopBackground.Id, OffsetX = 630, OffsetY = -395 })

	local color = Color.White
	if bloodCost <= 0 then
		color = Color.CostUnaffordable
	elseif (not bloodRefundConfig.FreeMode) and (not HasResource("LockKeys", keyCost)) then
		color = Color.CostUnaffordable
	end

	CreateTextBox({
		Id = components.RefundTotalLabel.Id,
    ...
    if not bloodRefundConfig.FreeMode then
		CreateTextBox({
			Id = components.RefundButton.Id,
			Text = "MetaUpgrade_Locked_Keys",
			FontSize = 19,
			Color = color,
			Font = "AlegreyaSansSCBold",
			OffsetX = 35,
			LuaKey = "TempTextData",
			LuaValue = { Amount = keyCost }, -- IMPORTANT: use keyCost (or amount), not a stale variable
		})
	end
...
function AspectExtender.WeaponUpgradeScreenLoadPage(screen)
	...
	for i = screen.StartingIndex, screen.StartingIndex + 4 - 1 do
		...
		if itemData ~= nil then
			...
			if IsWeaponUpgradeEquipped( weaponName, itemIndex ) then
				SetAnimation({ Name = "EquippedIcon", DestinationId = components[purchaseButtonKey.."Subtitle"].Id })
				TeleportCursor({ OffsetX = ScreenCenterX + 40, OffsetY = 20 + offsetIndex * 220, ForceUseCheck = true })
			end
			...
			if not IsUpgradeWeaponUpgradeDisabled( weaponName, itemIndex ) then
				...
				if not CanUpgradeWeaponUpgrade( weaponName, itemIndex ) then
    				...
    				UseableOn({ Id = components[purchaseButtonKey .. "Upgrade"].Id })
				end
				if not HasResource("SuperLockKeys", GetNextWeaponUpgradeKeyCost( weaponName, itemIndex )) then
					color = Color.CostUnaffordable
				end
...
function RefundBlood(screen, button)
    ...
    local bloodCost = button.BloodRefundCost
    ...
    if not cfg.FreeMode and not HasResource("LockKeys", keyCost) then
		return
	end
    ...
    for itemIndex, _ in pairs( WeaponUpgradeData[weaponName] ) do
    ...
    if not cfg.FreeMode then
		SpendResource( "LockKeys", keyCost )

		wait(0.4)

		AddResource( "SuperLockKeys", bloodCost )
	else
		return
	end
end
